...
namespace TSK.Controllers
{
    ...
    public class PagoPendPagoController : Controller
    {
        private SPPEU2GIGDEVSQLContext _context;

        public PagoPendPagoController(SPPEU2GIGDEVSQLContext context) {
            _context = context;
        }
        ...
        [HttpGet]
        public async Task<IActionResult> EstadoFilterLookup(DataSourceLoadOptions loadOptions)
        {
            var lookup = from i in _context.Estados
                         where (i.IdEstado == 2 || i.IdEstado== 3)
                         orderby i.NombreEstado
                         select new
                         {
                             Value = i.IdEstado,
